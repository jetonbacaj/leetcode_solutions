...
### file%3A%2F%2F%2Fhome%2Fr00t%2Fcode%2Fleetcode_solutions%2Fhard%2FmedianFromDataStream%2Fsrc%2Fmain%2Fscala%2Fexample%2FSolution.scala:35: error: `end of file` expected but `}` found
}
^
...
object Solution {
    ...
    class MedianFinder() {
        ...
        def findMedian(): Double = {
            ...
            if(arr.length % 2 == 0){
                (arr(arr.length/2-1) + arr(arr.length/2)).doubleValue() / 2
            }

            arr(arr.length/2-1).doubleValue()
        }
    }
